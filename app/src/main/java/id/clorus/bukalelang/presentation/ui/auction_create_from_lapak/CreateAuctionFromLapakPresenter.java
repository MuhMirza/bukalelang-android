...
import static id.clorus.bukalelang.R.id.weight;

/**
 * Created by mirza on 24/05/17.
 */

public class CreateAuctionFromLapakPresenter {
    ...
    public CreateAuctionFromLapakPresenter(CreateAuctionView view, Context context){
        this.view = view;
        this.context = context;
        appPreference = Esperandro.getPreferences(AppPreference.class, context);
    }

    public void createAuctionRequest(String productId,
                                     int minPrice,int kelipatanBid,String endDate){
        ...
        RestService.Factory.getInstance().createAuctionFromLapak(appPreference.id(),appPreference.bukalapakId(),appPreference.accessToken(),productId,minPrice,kelipatanBid,endDate).enqueue(new Callback<CreateAuctionData>() {
            @Override
